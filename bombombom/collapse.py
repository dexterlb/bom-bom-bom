...
def _collapse_field(items, collapser):
    if collapser['type'] == 'join':
        return collapser['by'].join((str(item) for item in sorted_h(items, collapser['sort_type'])))
    elif collapser['type'] == 'group_by_prefix':
        prefix_lists = defaultdict(list)
        for item in items:
            k, v = item.split(collapser['split_prefix_by'], 1)
            prefix_lists[k] += [v]
        prefix_strings = {k: collapser['join_values_by'].join(sorted_h(vs, collapser['sort_type'])) for k, vs in prefix_lists.items()}
